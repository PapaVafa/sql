...
Select * 
FROM
	vendor INNER JOIN vendor_booth_assignments
...
SELECT 
    ...
    SUM(customer_purchases.cost_to_customer_per_qty*customer_purchases.quantity) AS total_purchase
FROM customer 
...
FROM 
    customer_purchases
WHERE 
    strftime('%Y', transaction_time) = '2022' 
    AND strftime('%m', transaction_time) = '04'
GROUP BY 
    customer_id;
